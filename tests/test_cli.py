...
def test_cli_resolve_command() -> None:
    result = runner.invoke(app, ["resolve", "latex paint", "--ontology-root", str(ONTOLOGY_ROOT)])

    assert result.exit_code == 0
    assert '"canonical": "emulsion paint"' in result.stdout
...
def test_cli_init_concept(tmp_path: Path) -> None:
    ...
    assert result.exit_code == 0
    concept_path = tmp_path / "paint" / "V1" / "test-concept.md"
    metadata_path = tmp_path / "paint" / "V1" / "ontology.json"
...
def test_cli_can_select_paint_area() -> None:
    result = runner.invoke(
        app,
        [
            "resolve",
